...
function scrollToTop() {
	$(this).animate({scrollTop: $('#regformOR').offset().top});
}
...
*/ 
	...
	$('#ident a.connected').hover(function(){
	$(this).siblings('ul').show().delay(1000).show();
	}, function(){
	$(this).siblings('ul').delay(1000).hide();
	});
...
openRegister = function(site,uid,mail,name) {
	...
	if($("#"+idL).length > 0) {
		...
		//JSON direct sample
	   	$.get("/toolbar/reactions/proxy.php?action=openRegister&site="+site+"&uid="+uid+"&mail="+encodeURIComponent(mail)+"&name="+encodeURIComponent(name), { target: window.location.pathname }, function(data) {
		// if there is data, filter it and render it out
			if(data) {
...
animateButton = function() {
	//animate under submission button
	$('#go').click(function() {
	  $('html').animate({scrollTop: $('#regformOR').offset().top});
	});
}
...
buildDialog = function($dialog, data, id) {
	...
	$("#fgpwdl").attr("href","http://pdf.lesoir.be/authentification/forget.php?aff=FULL&amp;backurlPDF="+encodeURIComponent(window.location.href));
	$("#fgpwdl2").attr("href","http://pdf.lesoir.be/mon_profil/register.php?aff=FULL&amp;backurlPDF="+encodeURIComponent(window.location.href));
	$dialog.dialog('open');
	$("#memail").focus();
	return false;
}
...
coPr = function(login,password) {
	...
	function(data){
		if(data[0]=="ERROR") {
			...
			//$('#regformOC').html('<div class="p-main-content"><div class="p-wrap"><div class="p-the-content"><h1 id="titerr">'+ data[1] +'</h1><div id="divtochange"><div id="loading">&nbsp;</div><form name="mydata" id="mydatal" class="JeMeConnecte" action="" method="post" onsubmit=\'return coPr(document.getElementById(\"memail\").value, document.getElementById(\"mmdp\").value);\'><input type="hidden" name="form" value="done" /><div class="l-new-msg"><p><strong>Nouveau</strong>: la proc&eacute;dure de connexion a &eacute;t&eacute; modifi&eacute;e.</p><a id="l-knowMore" href="http://pdf.lesoir.be/mon_profil/faq">Mettre &agrave; jour mon profil</a></div> <div class="row"><label for="memail"><span>*</span> E-mail :</label>	<input type="text" class="form-text" name="memail" id="memail" /></div>	<div class="row"><label for="mmdp"><span>*</span> Mot de passe :</label><input type="password" class="form-text" name="mmdp" id="mmdp" /></div><input type="submit" class="btn-submit" value="je r&eacute;essaie" /></form><div class="row" id="fgpwd"><a id="fgpwdl" href="http://pdf.lesoir.be/authentification/forget.php?aff=FULL&amp;backurlPDF='+encodeURIComponent(window.location.href)+'">J\'ai oubli&eacute; mon mot de passe</a><a id="fgpwdl2" href="http://pdf.lesoir.be/mon_profil/register.php?aff=FULL&backurlPDF='+encodeURIComponent(window.location.href)+'">Je m\'inscris</a></div></div></div><div class="p-bottom">&nbsp;</div></div></div><iframe id="ssol" style="display: none;"></iframe>');
			...
			if ($.browser.msie) {
				//document.getElementById("ssol").src=url;
				window.location.replace(url);
			}else
			{
				$("#ssol").attr('src',url);
			}
			$("#fgpwd").html("Si votre page ne se rafraichit pas dans les 5 secondes, veuillez <a href='#' onClick='window.location.reload( true );'>cliquer ici</a>");
		}
	}, "json");
