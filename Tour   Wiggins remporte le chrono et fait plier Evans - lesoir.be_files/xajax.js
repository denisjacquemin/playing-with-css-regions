...
this.loadingFunction=function(){};this.doneLoadingFunction=function(){};var loadingTimeout;this.call=function(sFunction,aArgs,sRequestType,sWrapperPrefix){var i,r,postData;if(document.body&&xajaxWaitCursor)
...
var uri = eval(sWrapperPrefix+"RequestUri");var value;switch(xajaxRequestType){case xajaxDefinedGet:{var uriGet=uri.indexOf("?")==-1?"/xajax="+encodeURIComponent(sFunction):"/xajax="+encodeURIComponent(sFunction);if(aArgs){for(i=0;i<aArgs.length;i++){value=aArgs[i];if(typeof(value)=="object")
value=this.objectToXML(value);uriGet+="/xajaxargs[]="+encodeURIComponent(value);}
};uri+=uriGet;postData=null;}break;case xajaxDefinedPost:{postData="xajax="+encodeURIComponent(sFunction);postData+="&xajaxr="+new Date().getTime();if (typeof validate == "undefined") { } else { postData+="&"+validate+"=1";}if(aArgs){for(i=0;i <aArgs.length;i++){value=aArgs[i];if(typeof(value)=="object")
...
return;if(r.status==200){if(xajaxDebug || 0==1)alert("Received:\n"+r.responseText);
newresponseXML = getXMLDocument( r );
if (newresponseXML ) {
xajax.processResponse(newresponseXML);
} else {var errorString="Error: the XML response that was returned from the server is invalid.";errorString+="\nReceived:\n"+r.responseText;trimmedResponseText=r.responseText.replace(/^\s+/g,"");trimmedResponseText=trimmedResponseText.replace(/\s+$/g,"");if(trimmedResponseText!=r.responseText)
...
try {
if( (browser == 'firefox' || browser == 'navigator' || browser == 'camino' || browser == 'netscape' || browser == 'gecko' || browser == 'opera') && document.domain == 'coveritlive.com'  ) {
